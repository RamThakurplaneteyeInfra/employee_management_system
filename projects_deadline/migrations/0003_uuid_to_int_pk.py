"""
Switch DeadlineProject + DeadlineProjectPhase from UUID primary keys
to simple auto-incrementing integer IDs.

Since there is no production data yet, we drop and recreate the tables.
The old project."ProjectPhase" table is NOT affected (still unmanaged).
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("projects_deadline", "0002_new_deadline_models"),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        # Drop the UUID-based tables (no real data exists yet)
        migrations.DeleteModel(name="DeadlineProjectPhase"),
        migrations.DeleteModel(name="DeadlineProject"),

        # Recreate with auto-incrementing integer PK
