...
class LeaveApplicationViewSet(ModelViewSet):
    ...
    def performance_score(self, request):
        """
        Combined leave + meeting + checklist + certification performance score for one employee.
        ...
        Response includes nested `leave`, `meeting`, `checklist`, and `certification` payloads plus
        `combined_total_points` (sum of all four total_points).
        """
        year, month, quarter, period_err = parse_leave_points_period(request)
        if period_err is not None:
            return Response(period_err, status=status.HTTP_400_BAD_REQUEST)
